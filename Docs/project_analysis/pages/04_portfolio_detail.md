# 화면 기획서: 포트폴리오 상세

> **Screen ID**: `screen-detail`
> **Phase**: 1 (MVP)
> **우선순위**: P1
> **마지막 수정**: 2026-01-04

---

## 📋 화면 개요

| 항목                     | 내용                                                |
| ------------------------ | --------------------------------------------------- |
| **화면명**         | 포트폴리오 상세                                     |
| **목적**           | 개별 포트폴리오의 종목별 현황 및 비율 확인          |
| **진입 경로**      | 홈 → 포트폴리오 카드 탭                            |
| **이동 가능 화면** | ← 홈, → 종목 검색, → 리밸런싱 분석, → 편집 모달 |

### 사용자 스토리

> "투자자로서 나는 각 종목의 **현재 비율과 목표 비율의 차이**를 직관적으로 파악하고, 리밸런싱이 필요한지 판단하고 싶다."

---

## 🎯 핵심 사용자 니즈

| 니즈                        | 중요도 | 현재 충족 여부   |
| --------------------------- | ------ | ---------------- |
| 종목별 비율 vs 목표 비교    | ⭐⭐⭐ | ✅ (비율 바)     |
| 포트폴리오 총 수익 확인     | ⭐⭐⭐ | ✅               |
| 종목별 수익률 확인          | ⭐⭐   | ✅               |
| 리밸런싱 필요성 빠른 판단   | ⭐⭐⭐ | ⚠️ (개선 가능) |
| 종목 상세 정보 (수량, 단가) | ⭐     | ❌ (미구현)      |

---

## 🧩 현재 구현 요소

```
┌─────────────────────────────────────┐
|  ←  메인 포트폴리오              ≡  |  <-- 햄버거 메뉴 (알림 설정, 편집)
├─────────────────────────────────────┤
│ ┌─────────┬─────────┐              │
│ │현금 비중 │총 매입금  │              │
│ │₩1.6M(5%)│₩30.25M  │              │
│ ├─────────┼─────────┤              │
│ │ 수익금   │ 수익률   │              │
│ │+₩2.2M   │ +7.27%  │              │
│ └─────────┴─────────┘              │
├─────────────────────────────────────┤
│ 보유 종목      5종목  비율합계 100% │
├─────────────────────────────────────┤
│ ⚠️ 리밸런싱 필요                    │
│ 삼성전자 +17% 초과 · 카카오 -20% 미달│
│                        [분석 →]    │
├─────────────────────────────────────┤
│ ┌─────────────────────────────────┐ │
│ │ 삼성전자           ₩11,357,500 │ │
│ │ 005930                  +5.2%  │ │
│ │ 현재 35%   ━━━|███   목표 30%  │ │
│ │           [+17% 초과]          │ │
│ └─────────────────────────────────┘ │
│ ┌─────────────────────────────────┐ │
│ │ SK하이닉스          ₩8,112,500 │ │
│ │ 000660                  +8.1%  │ │
│ │ 현재 25%   ━━━|━━━   목표 25%  │ │
│ │           [적정 범위 ✓]   (×)  │<-- 편집 모드 시 삭제 버튼 노출
│ └─────────────────────────────────┘ │
│         ... 이하 종목 카드 ...      │
├─────────────────────────────────────┤
│ [+ 종목 추가]   [⚖️ 리밸런싱 분석]   │
└─────────────────────────────────────┘
```

**구현된 기능:**

- ✅ Summary Grid (현금 비중 포함 4개 지표)
- ✅ 카드 형태 종목 리스트
- ✅ 초과/미달 바 (Deviation Bar)
- ✅ 리밸런싱 인사이트 배너
- ✅ 페이즈(Phase) 1 인터랙션:
  - **헤더 메뉴 (≡)**: 알림 설정, 편집 모드 진입
  - **알림 설정 모달**: 포트폴리오별 전체 알림 ON/OFF (중앙 모달)
  - **편집 모드**: 헤더 '완료' 버튼 표시, 각 종목 카드에 '삭제(×)' 버튼 노출

## 💡 개선 아이디어

### 1. 종목 클릭 시 상세 정보

┌───────────────────────────────────────────┐
│ 삼성전자 005930                    [X]   │
├───────────────────────────────────────────┤
│ 현재가         ₩74,500     +1.2% 🔺     │
│ 보유 수량      150주                      │
│ 평균 매입가    ₩71,200                   │
│ 평가금액       ₩11,175,000               │
│ 수익금         +₩495,000 (+4.6%)         │
├───────────────────────────────────────────┤
│ 현재 비율      35%                        │
│ 목표 비율      30%                        │
│ 괴리           +5%p (임계값 ±5% 초과)     │
├───────────────────────────────────────────┤
│ [수정]              [삭제]                │
└───────────────────────────────────────────┘

### 3. 종목 정렬 옵션 (Phase 2)

정렬: [비율순 ▼]
      - 비율순 (높은순) ← 기본
      - 금액순 (높은순)
      - 수익률순 (높은순)
      - 이름순

## ⚠️ 엣지 케이스 및 에러 상태

| 상황              | 현재 처리 | 권장 처리                   |
| ----------------- | --------- | --------------------------- |
| 종목 0개          | ✅ Empty State | 종목 추가 CTA 연결 |
| 종목 1개          | ❌ 없음   | 리밸런싱 불가 안내          |
| 비율 합계 ≠ 100% | ❌ 없음   | 경고 표시                   |
| 시세 조회 실패    | ❌ 없음   | 마지막 캐시 + 오류 표시     |
| 상장폐지 종목     | ❌ 없음   | 경고 배지 + 삭제 유도       |
| 편집 모드 해제    | ✅ 완료 버튼 | 자동 저장 또는 확인 팝업 |

### Empty State (종목 0개)

┌─────────────────────────────────────┐
│                                     │
│              📈                     │
│                                     │
│     아직 종목이 없어요              │
│   첫 종목을 추가해보세요!          │
│                                     │
│       [+ 종목 추가]                 │
│                                     │
└─────────────────────────────────────┘

## 🔄 화면 상태 (States)

### 1. Loading State

- Summary Grid: 스켈레톤
- 종목 리스트: 스켈레톤 3줄

### 2. Editing Mode (상태 토글)

- **진입**: 헤더 메뉴(≡) > [편집] 선택
- **화면**: 헤더 우측 '완료' 버튼 표시, 햄버거 메뉴 숨김
- **종목 카드**: 우측 상단 빨간색 원형 삭제(×) 버튼 팝업 애니메이션
- **종료**: '완료' 버튼 클릭 시 일반 모드로 복귀

---

## 📐 테이블 vs 카드 레이아웃

| 종목 수  | 권장 레이아웃             |
| -------- | ------------------------- |
| 1~5개    | 테이블 (밀도 높음)        |
| 6개 이상 | 카드 리스트 (스크롤 명확) |

### 모바일 최적화

- 테이블 헤더: 고정 (sticky)
- 가로 스크롤: 없음 (정보 압축)
- 비율 바: 너비 고정 (80px)

---

## ✅ 체크리스트

### MVP (Phase 1)

- [x] 헤더 (뒤로가기, 타이틀, 편집) `[직접]`
- [x] 포트폴리오 요약 카드 `[직접]`
- [x] 종목 리스트 테이블/카드 `[직접]`
- [x] 종목 클릭 상세 정보 `[직접]`
- [x] **종목 삭제 기능** (편집모드) `[직접]`
- [x] **현금 자산 표시** & 비중 계산 `[직접]`
- [x] **포트폴리오별 임계값/알림 설정** (헤더 메뉴) `[직접]`
- [x] 리밸런싱 인사이트 배너 `[직접]`
- [x] FAB (종목 추가, 리밸런싱) `[직접]`
- [x] Empty State `[패널]`
- [x] **헤더 메뉴/드롭다운 & UI 개선** `[직접]`
- [ ] Pull-to-Refresh (지정된 주기) `[패널]` 종목별 메모 기능

### Phase 2

- [ ] 종목 정렬 옵션
- [ ] 종목별 메모 기능

---

## 🔗 관련 문서

- [03_home.md](./03_home.md) - 이전 화면 (홈)
- [05_stock_search.md](./05_stock_search.md) - 종목 추가
- [06_rebalancing.md](./06_rebalancing.md) - 리밸런싱 분석
- [ux_design_recommendations.md](../ux_design_recommendations.md) - 비율 표현 상세 가이드
